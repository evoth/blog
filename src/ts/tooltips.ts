...
// Anyways, these objects store lists of classes to be applied to the
// corresponding elements, depending on whether the tooltip is on the left/right,
// top/bottom, and whether it's open or closed.
...
const baseClasses: StateClasses = {
  left: ["right-0"],
  right: ["left-0"],
  top: [],
...
const arrowClasses: StateClasses = {
  left: ["left-2"],
  right: ["right-2"],
...
function editClasses(
  ...
  ...elementAndClasses: [
    element: HTMLElement,
    addClasses: StateClasses,
    removeClasses?: StateClasses
  ][]
) {
...
function tooltipEnter(event: MouseEvent) {
  ...
  const anchor = tooltip.getElementsByClassName(
    "tooltip-anchor"
  )[0] as HTMLElement;
  // Element to be positioned on corner of tooltip label text
  const base = anchor.getElementsByClassName("tooltip-base")[0] as HTMLElement;
  // Body of tooltip, extends inwards horizontally and vertically
  const body = base.getElementsByClassName("tooltip-body")[0] as HTMLElement;
  // Arrow that points to label text; inset from outer edge of tooltip body
  const arrow = base.getElementsByClassName("tooltip-arrow")[0] as HTMLElement;
...
for (let i = 0; i < tooltips.length; i++) {
  const body = tooltips[i].getElementsByClassName(
    "tooltip-body"
  )[0] as HTMLElement;
  const link = tooltips[i].getElementsByTagName("a")[0] as HTMLElement;
